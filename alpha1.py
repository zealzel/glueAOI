...
def parse_image(
    ...
    cropped_image0 = image[y_start:y_end, x_start:x_end]

    if show:
        cv2.namedWindow("show", cv2.WINDOW_NORMAL)
        cv2.imshow("show", cropped_image)
        cv2.resizeWindow("show", 500, 800)
        #
        cv2.namedWindow("show0", cv2.WINDOW_NORMAL)
        cv2.imshow("show0", cropped_image0)
        cv2.resizeWindow("show0", 500, 800)

        # 将show0窗口移动到show窗口的右侧
        cv2.moveWindow("show0", 520, 100)  # 根据需要调整 (x, y) 的值，x为水平偏移量
